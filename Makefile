...
init: | $(BUILDDIR) $(SRCDIR) $(INPUTDIR) $(OUTPUTDIR)
	...
	@echo "\n"																													>> $(SRCDIR)/$(TARGET)$(SRCEXT)	
	@echo "using namespace std;"																								>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo ""																													>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "class Main {"																										>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "public:"																												>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "    int32_t t{1};"																									>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo ""																													>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "    inline void solve() {"																							>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "        // TODO: code here"																							>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "	}"																													>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "    void main() {"																									>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "        ios_base::sync_with_stdio(false);"																			>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "        cin.tie(nullptr);"																							>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "        /* comment this line if there is no multiple test cases */"													>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	...
	@echo "    Main obj;"																										>> $(SRCDIR)/$(TARGET)$(SRCEXT)
	@echo "    obj.main();"																										>> $(SRCDIR)/$(TARGET)$(SRCEXT)
